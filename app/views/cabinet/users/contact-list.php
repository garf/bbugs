            ...
            <tr class="contact<?= $contact->coid ?>">
                <td class="col-md-1 text-center">
                    <img class="img-thumbnail"
                         alt="Gravatar" src="<?= e(Gravatar::src($contact->email, 64)) ?>">
                    <br />
                    <div style="margin-top: 5px;">
                        ...
                        <a href="<?= URL::route('contact-delete', array($contact->coid)) ?>"
                           ...
                           data-message="Удалить <?= e($contact->title) ?>?"
                ...
                <td>
                    <div class="clearfix">
                        <div class="text-left">
                            <strong><?= trim($contact->title) != '' ? e($contact->title) : e($contact->name) ?></strong>
                            <span class="text-muted"><?= trim($contact->title) != '' ? e($contact->name) : '' ?></span>
                            ...
                            <?php } ?>
                            <?php if ($contact->is_phone && trim($contact->phone) != '') { ?>
                            <div>
                                <span class="text-muted"><?= trans('users.phone') ?>:</span>
                                +<?= e($contact->phone) ?>
                            </div>
                            <?php } ?>
                            <?php if ($contact->is_skype && trim($contact->skype) != '') { ?>
                            <div>
                                <span class="text-muted"><?= trans('users.skype') ?>:</span>
                                <a href="skype:<?= e($contact->skype) ?>">
                                    <?= e($contact->skype) ?>
                                </a>
                            </div>
                            <?php } ?>
                            <div><?= e($contact->notes) ?></div>
